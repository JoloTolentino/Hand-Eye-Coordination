...
class Hard_Voting:
    ...
    def verificaiton(self,data):
        if len(self.memory) < 2: self.memory.insert(0,data[0]);self.Condition = False
        self.memory.insert(0,data[0])
        self.Condition = True if len(np.unique(self.memory))==1 else False
